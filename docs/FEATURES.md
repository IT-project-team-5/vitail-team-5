...
| Redemption | Ready for device test | Café-first catalogue and menus with Maps links; order details use slide-to-collect. One reward/quantity 1 per order; owner confirmation deducts once and returns a reference; retry UUID is idempotent; only that owner can collect; history includes terminal states |
