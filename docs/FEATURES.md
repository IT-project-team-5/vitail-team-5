...
| App navigation | Ready for device test | Owner sees Account/Walk/Redeem with real wallet points; café sees Account/Products/Orders; logout stays in Account |
...
| Café venue profile | Ready for device test | Café Account edits its own name/address/description/opening hours; name uses the same user display name as the catalogue, email stays read-only; products/prices are managed separately in Products |
