...
| Café orders | Ready for device test | Item-first cards show customer and current profile dogs, with details on tap. Feed remains read-only and café-scoped. Profile-aware clients request fresh pending-order snapshots; legacy deltas/304 remain supported |
