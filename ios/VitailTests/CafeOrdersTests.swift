...
final class CafeOrdersTests: XCTestCase {
    ...
    @MainActor
    func testCancellingVisiblePollingStopsBeforeAnotherRequest() async {
        let service = CafeOrdersServiceStub(responses: [
            .result(.updated(CafeOrdersFeed(cursor: 1, orders: [], removedOrderIDs: [])))
        ])
        let model = CafeOrdersViewModel(service: service)
        let poll = Task { await model.pollWhileVisible() }
        for _ in 0..<100 where !model.hasLoaded { await Task.yield() }
        poll.cancel()
        await poll.value
        let cursors = await service.receivedCursors()
